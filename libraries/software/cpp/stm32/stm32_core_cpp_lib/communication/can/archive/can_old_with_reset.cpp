 ...
 *      Author: lehmann
 *
 *  Modified: Added reset() to clear stale state on CAN_READING_ERROR.
 */
...
/* ---------------------------------------------------------------------------- */
// New reset function to recover from a hang or stale state
// Call this function when you detect a CAN_READING_ERROR to reinitialize the CAN module.
void CAN::reset() {
    // Stop the CAN hardware to clear any ongoing operations.
    HAL_FDCAN_Stop(this->config.hfdcan);

    // Clear any pending read requests to flush stale responses.
    xSemaphoreTake(this->mapMutex, portMAX_DELAY);
    for (int i = 0; i < CAN_NUMBER_RR; i++) {
        readRequests[i].isOpen = false;
        readRequests[i].taskHandle = nullptr;
    ...
    // (Optional) If supported by your HAL, flush any messages in the FIFOs here.

    // Restart the CAN hardware so that listening resumes.
