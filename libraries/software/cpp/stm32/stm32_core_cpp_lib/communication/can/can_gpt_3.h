...
#ifndef COMMUNICATION_CAN_CAN_GPT_3_H_
#define COMMUNICATION_CAN_CAN_GPT_3_H_
...
struct CallbackEntry {
	CANFrameCallback callback; // The callback function
...
public:
	...
	CANManager() {
		...
		for (int i = 0; i < CAN_NUMBER_CALLBACKS; i++) {
			standardIDCallbacks[i].callback = nullptr;
	...
	bool registerStandardIDCallback(CANFrameCallback callback,
		...
		xSemaphoreTake(mapMutex, portMAX_DELAY);
		for (int i = 0; i < CAN_NUMBER_CALLBACKS; i++) {
			if (standardIDCallbacks[i].callback == nullptr) {
				standardIDCallbacks[i].callback = callback;
				standardIDCallbacks[i].FilterID1 = FilterID1;
				standardIDCallbacks[i].FilterID2 = FilterID2;
				xSemaphoreGive(mapMutex);
	...
	bool registerExtendedIDCallback(CANFrameCallback callback,
		...
		xSemaphoreTake(mapMutex, portMAX_DELAY);
		for (int i = 0; i < CAN_NUMBER_CALLBACKS; i++) {
			if (extendedIDCallbacks[i].callback == nullptr) {
				extendedIDCallbacks[i].callback = callback;
				extendedIDCallbacks[i].FilterID1 = FilterID1;
				extendedIDCallbacks[i].FilterID2 = FilterID2;
				xSemaphoreGive(mapMutex);
				return true; // Callback registered successfully
			}
		}
	...
	// Remove a callback for 11-bit CAN frames
	void removeStandardIDCallback(CANFrameCallback callback) {
		xSemaphoreTake(mapMutex, portMAX_DELAY);
		for (int i = 0; i < CAN_NUMBER_CALLBACKS; i++) {
			if (standardIDCallbacks[i].callback == callback) {
				standardIDCallbacks[i].callback = nullptr;
				break;
			}
		}
	...
	// Remove a callback for 29-bit CAN frames
	void removeExtendedIDCallback(CANFrameCallback callback) {
		xSemaphoreTake(mapMutex, portMAX_DELAY);
		for (int i = 0; i < CAN_NUMBER_CALLBACKS; i++) {
			if (extendedIDCallbacks[i].callback == callback) {
				extendedIDCallbacks[i].callback = nullptr;
				break;
			}
		}
	...
	void onMessageReceived(const FDCAN_RxHeaderTypeDef &header, uint8_t *data) {
		...
		for (int i = 0; i < CAN_NUMBER_RR; i++) {
			if (readRequests[i].isOpen
					&& readRequests[i].id == header.Identifier) {
		...
		if (header.IdType == FDCAN_STANDARD_ID) {
			for (int i = 0; i < CAN_NUMBER_CALLBACKS; i++) {
				if (standardIDCallbacks[i].callback != nullptr
						...
						&& header.Identifier
								<= standardIDCallbacks[i].FilterID2) {
					standardIDCallbacks[i].callback(header.Identifier, data,
		...
		if (header.IdType == FDCAN_EXTENDED_ID) {
			for (int i = 0; i < CAN_NUMBER_CALLBACKS; i++) {
				if (extendedIDCallbacks[i].callback != nullptr
						...
						&& header.Identifier
								<= extendedIDCallbacks[i].FilterID2) {
					extendedIDCallbacks[i].callback(header.Identifier, data,
							header.DataLength >> 16);
				}
			}
		}
...
HAL_StatusTypeDef sendMessage(FDCAN_HandleTypeDef *hfdcan, uint32_t id,
		uint8_t *data, uint8_t length, bool isExtendedID = true) {
	FDCAN_TxHeaderTypeDef TxHeader = { .Identifier = id, .IdType =
			isExtendedID ? FDCAN_EXTENDED_ID : FDCAN_STANDARD_ID, .TxFrameType =
			FDCAN_DATA_FRAME,
			.DataLength = length << 16, // Convert length to DLC
			.ErrorStateIndicator = FDCAN_ESI_ACTIVE, .BitRateSwitch =
					FDCAN_BRS_OFF, .FDFormat = FDCAN_CLASSIC_CAN,
...
CAN_Status sendRemoteFrame(FDCAN_HandleTypeDef *hfdcan, uint32_t id,
		uint32_t timeoutMs, uint8_t *responseData, uint8_t &responseLength) {
	...
	// Send Remote Frame
	FDCAN_TxHeaderTypeDef TxHeader = { .Identifier = id, .IdType =
			FDCAN_EXTENDED_ID, .TxFrameType = FDCAN_REMOTE_FRAME,
	...
	if (xSemaphoreTake(responseSemaphore, pdMS_TO_TICKS(timeoutMs)) == pdTRUE) {
		...
		for (int i = 0; i < CAN_NUMBER_RR; i++) {
			if (readRequests[i].id == id && !readRequests[i].isOpen) {
				...
				memcpy(responseData, readRequests[i].responseData,
						readRequests[i].responseLength);
...
#endif /* COMMUNICATION_CAN_CAN_GPT_3_H_ */
