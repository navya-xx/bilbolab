...
class SingletonLock:
    ...
    def __init__(self, lock_file="default.lock", override=False, timeout=None, mode=None, override_timeout=1):
    ...
    def __enter__(self):
        ...
        while True:
            try:
                    ...
                    with open(self.lock_file, "r") as f:
                        ...
                        if process_running(pid):
                            if self.override:
                                ...
                                time.sleep(self.override_timeout)  # Allow time for the process to clean up
                                elapsed = 0
                ...
                self.lock_fd.flush()
                print(f"Lock \"{self.lock_file}\" acquired by \"{self.script}\" with mode \"{self.mode}\"")
                break
            except IOError:
...
def check_for_running_lock_holder(lock_file):
    """
    Checks if the lock file exists and if the PID in the lock file is running.

    Args:
        lock_file (str): The path to the lock file.

    Returns:
        bool: True if the lock file exists and the PID is running, False otherwise.
    """
    resolved_path = resolve_lock_file_path(lock_file)
        ...
        try:
            with open(resolved_path, "r") as f:
                ...
                if process_running(pid):
                    ...
                    print(f"Removing stale lock file \"{resolved_path}\".")
                    os.remove(resolved_path)
                    return False
        except Exception as e:
            print(f"Error checking lock file \"{resolved_path}\": {e}")
            return False
    return False
