...
class StatusWidget(Widget):
    ...
    """
    def __init__(self, id, items=None, color=None, textcolor=None,
                 size=None, position=None, lockable=False, locked=False):
        super().__init__(id, text="", color=color, textcolor=textcolor,
                         size=size, position=position,
                         lockable=lockable, locked=locked)
        self.items = items if items else []

    def get_payload(self):
        base_payload = super().get_payload()
...
class GraphWidget:
    def __init__(self, id, title, y_min, y_max, window_time, color, textcolor="#fff",
...
def example():
    ...
    root_group = ButtonGroup("Root", group_type="folder", pages=4)
    btn1 = root_group.addWidget(Widget("btn1", "Button ❌", [1, 1, 1], textcolor=[1, 0, 0], lockable=True, locked=True))
    ...
    status_widget = StatusWidget(
        "status1",
        items=[
            {"marker_color": "#FF0000", "name": "Engine", "status": "a"},
            {"marker_color": "#00FF00", "name": "Pump", "status": "Running"},
            {"marker_color": "#FFFF00", "name": "Sensor", "status": "Warning"},
        ],
        color=[0.2, 0.2, 0.2],
        textcolor=[1, 1, 1],
        size=(2, 2)
    )
    root_group.addWidget(status_widget)
    ...
    graph_widget = GraphWidget("graph1",
                               ...
                               x_ticks_spacing=1,
                               window_time=10,  # 10 seconds rolling window
                               color=[0.2,0.2,0.2],
                               textcolor="#FFFFFF",
                               line_color=[0,1,1],
                               size=(2, 1),
    ...
    graph_widget2 = GraphWidget("graph2",
                               "Real-Time Graph",
                               y_min=-100,
                               y_max=100,
                               y_ticks=[-90, -50, 0, 50, 90],
                               ...
                               line_color=[0,1,1],
                               size=(2, 1),
                               position=(1,5))  # minimum 2 columns wide

    graph_widget3 = GraphWidget("graph3",
                               "Real-Time Graph",
                               y_min=-100,
                               y_max=100,
                               y_ticks=[-90, -50, 0, 50, 90],
                               # y_ticks=[-50, -25, 0, 25, 50, 75, 100],
                               x_ticks_spacing=0.2,
                               window_time=1,  # 10 seconds rolling window
                               color=[0.2,0.2,0.2],
                               textcolor="#FFFFFF",
                               line_color=[0,0,1],
                               size=(3, 2),
                               position=(3,4))  # minimum 2 columns wide

    root_group.addWidget(graph_widget2)
    root_group.addWidget(graph_widget3)
    ...
    joystick = JoystickWidget("joy1", "JSA", 0, 0, size=(2, 2), fixed_axis="vertical", lockable=True, locked=False)
    ...
    def push_graph_data():
        ...
        while True:
            ...
            time.sleep(0.01)
            graph_widget2.push_value(value)
            time.sleep(0.01)
            graph_widget3.push_value(value)
            t += 0.5
            time.sleep(0.1)
    ...
    try:
        t = 0  # Time variable
        while True:
            value = 100 * math.sin(t)  # Sinusoidal value between -100 and 100
            digital_number.set_value(value)
            t += 0.1  # Increase time step
            # app_instance.speak("HELLO")
            status_widget.update_value("Sensor", status=f"Val: {value:.2f}")
            time.sleep(1)
