...
class CentralizedLocationAlgorithm:
    ...
    def init(self, agents: dict[str, VisionAgent]):
        ...
        for i, agent in enumerate(agents.values()):
            agent_state = self.augmentAgentState(agent.state)
            self.state[i * AGENT_STATE_DIM:(i + 1) * AGENT_STATE_DIM] = agent.state
        ...
        for i, agent in enumerate(agents.values()):
            agent_covariance = self.augmentAgentCovariance(agent.state_covariance)
