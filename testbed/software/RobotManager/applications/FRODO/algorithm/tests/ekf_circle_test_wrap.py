...
def h_23(X):
    ...
    # meas_rot = qmt.wrapToPi(meas_rot)
...
def ekf_update(X, P, z, R_meas, h_func):
    ...
    # Compute innovation
    y = z - z_pred
    # For each index corresponding to an angle, wrap the innovation.
    for i in range(len(y)):
        if i % 3 == 2:
            y[i] = qmt.wrapToPi(y[i])
    # Compute the Jacobian on the smooth, unwrapped h_func
...
def main():
    ...
    for step in range(num_steps):
        ...
        #     if i % 3 == 2:
        #         z_true[i] = qmt.wrapToPi(z_true[i])
