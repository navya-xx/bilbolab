...
from applications.FRODO.algorithm.centralized_ekf import CentralizedLocationAlgorithm, VisionAgent, \
...
class FRODO_Algorithm_Simulated:
    ...
    def init(self,  agents: dict[str, dict]):
        ...
        for name, agent_data in agents.items():
            agent = self.simulation.addVirtualAgent(id = name, fov_deg = agent_data["fov_deg"], view_range=agent_data["view_range"])
        ...
        for name, agent in self.simulation.agents.items():
            vision_agents_algorithm[name] = VisionAgent(
                id = name,
                index = index,
                state = np.array([agent.state['pos']['x'], agent.state['pos']['y'], agent.state['psi'].value]) if agent.agent_id == 'frodo1_v' else np.array([0, 0, 0]),
                state_covariance = np.eye(3)*0 if agent.agent_id == 'frodo1_v' else np.eye(3)*1e5,
                input = np.array([0,0]),
