...
class Widget:
    def __init__(self, id, text, color: (str, list) = None, textcolor: (str, list) = None,
        ...
        self.grid_size = grid_size if grid_size is not None else (1, 1)
    ...
    def set_text(self, new_text):
        if self.app and self.app.loop:
            asyncio.run_coroutine_threadsafe(self._set_text(new_text), self.app.loop)
        else:
    ...
    def set_color(self, new_color):
        if self.app and self.app.loop:
            asyncio.run_coroutine_threadsafe(self._set_color(new_color), self.app.loop)
        else:
    ...
    def set_textcolor(self, new_textcolor):
        if self.app and self.app.loop:
            asyncio.run_coroutine_threadsafe(self._set_textcolor(new_textcolor), self.app.loop)
        else:
...
class MultiStateButton(Widget):
    def __init__(self, id, title, states, color=None, current_state=0, textcolor="#fff",
        ...
        super().__init__(id, title, color, textcolor, grid_size, position)
    ...
    def set_current_state(self, new_state):
        if self.app and self.app.loop:
            asyncio.run_coroutine_threadsafe(self._set_current_state(new_state), self.app.loop)
        else:
    ...
    def set_states(self, new_states):
        if self.app and self.app.loop:
            asyncio.run_coroutine_threadsafe(self._set_states(new_states), self.app.loop)
        else:
...
class MultiSelectButton(Widget):
    def __init__(self, id, name,  options, value, color=None, title="", textcolor="#fff",
                 grid_size=None, position=None):
        super().__init__(id, name, color, textcolor, grid_size, position)
    ...
    def set_value(self, new_value):
        if self.app and self.app.loop:
            asyncio.run_coroutine_threadsafe(self._set_value(new_value), self.app.loop)
        else:
...
class SliderWidget:
    def __init__(self, id, title, min_value, max_value, current_value, color, textcolor="#fff",
                 grid_size=None, position=None, direction="horizontal", automatic_reset=None):
        self.id = id
        self.title = title
        self.min = min_value
        self.max = max_value
...
class ButtonGroup:
    ...
    def removeWidget(self, widget_id):
        for child in self.children:
            if hasattr(child, "id") and child.id == widget_id and not isinstance(child, ButtonGroup):
    ...
    def get_payload(self):
        ...
        for child in self.children:
            if not hasattr(child, 'grid_size') or child.grid_size is None:
                child.grid_size = (1, 1)
            if child.position is not None:
                ...
                if col < 0 or row < 0 or col + w > grid_cols or row + h > grid_rows:
                    raise ValueError(
                        f"Widget {child.id} with size {w}x{h} at position ({col}, {row}) does not fit in the grid.")
                for r in range(row, row + h):
                    for c in range(col, col + w):
                        if occupancy[r][c]:
                            raise ValueError(f"Widget {child.id} overlaps with another widget at cell ({c}, {r}).")
                        occupancy[r][c] = True
            ...
            if child.position is None:
                ...
                for r in range(grid_rows):
                    for c in range(grid_cols):
                        if c + w <= grid_cols and r + h <= grid_rows:
                            can_place = True
                    ...
                    if placed:
                        break
                if not placed:
                    raise ValueError(f"Not enough space to place widget {child.id} with size {w}x{h}.")
        ...
        for child in self.children:
            ...
            if "doublewide" in payload:
                del payload["doublewide"]
            payload["position"] = [child.position[0], child.position[1]]
            payload["grid_size"] = [child.grid_size[0], child.grid_size[1]]
        ...
        grid_items = widget_payloads + placeholder_payloads
        ...
        return {
            ...
            "path": self.get_path(),
            "show_back": (self.group_type == "folder" and self.parent is not None)
        }
    ...
    def getButtonByPath(self, path):
        ...
        for token in tokens:
            ...
            for child in current.children:
                if hasattr(child, "id") and child.id == token:
                    found = child
                    break
            if found is None:
                return None
    ...
    # ------------------------------------------------------------------------------------------------------------------
    async def set_as_current_group(self, app_instance):
        app_instance.current_group = self
        await app_instance.broadcast_current_group()
...
class ControlApp:
                    ...
                    if data.get("type") == "button_click":
                        ...
                        if clicked is None:
                            ...
                            if isinstance(clicked, ButtonGroup):
                                await clicked.set_as_current_group(self)
                            elif isinstance(clicked, Widget):
                                await clicked.on_pressed()
                                if self.current_group.group_type == "modal" and self.current_group.parent:
                                    await self.go_back()
                        ...
                        if button_obj:
                            if isinstance(button_obj, Widget):
                                ...
                                result = {"id": button_obj.id, "name": button_obj.name, "color": button_obj.color,
                                          "is_folder": True}
                        else:
                            result = None
    ...
    async def go_back(self):
        if self.current_group.parent is not None:
            ...
            logger.debug(f"Returned to group: {self.current_group.name}")
    ...
    def run(self, host="0.0.0.0"):
        ...
        if Zeroconf is not None:
            ...
            logger.info(f"mDNS service registered as {self.mdns_name}.local")
        else:
    ...
    def setCurrentGroup(self, group: ButtonGroup):
        if self.loop:
            asyncio.run_coroutine_threadsafe(group.set_as_current_group(self), self.loop)
        else:
...
def example():
    app_instance = ControlApp(port=80, mdns_name='bilbocontrol')
    ...
    slider_widget = SliderWidget("slider1", "Volume", 0, 100, 50, "#0077CC", textcolor="#FFFFFF", grid_size=(3, 1))
    ...
    folder2.addWidget(Widget("f2btn1", "Folder Btn 1", "#FFAA00", grid_size=(3, 2), position=(1, 0)))
    folder2.addWidget(Widget("f2btn2", "Folder Btn 1", "#FFAA00", grid_size=(1, 2)))

    folder2.addWidget(SliderWidget("slider1", "Volume", 0, 100, 50, "#0077CC", textcolor="#FFFFFF", grid_size=(1, 2)))
    ...
    slider_h = SliderWidget("slider_h", "Horizontal Slider", -100, 100, 0, "#00CC00", textcolor="#FFFFFF",
                            grid_size=(2, 2), direction="vertical", automatic_reset=0)
    slider_v = SliderWidget("slider_v", "Vertical Slider", -100, 100, 0, "#CC0000", textcolor="#FFFFFF",
                            position=(3, 0), grid_size=(2, 2), direction="horizontal", automatic_reset=0)
