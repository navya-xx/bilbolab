...
class BILBO_GUI_NODE_JS(NodeJSGui):
    gui_joystick_array: []
    ...
    def addJoystick(self, added_joystick: Joystick):
        for joystick in self.gui_joystick_array:
            if joystick.get('id') == added_joystick.id:
                return

        joystick = {"id": added_joystick.id, "name": added_joystick.name, "assignedBot": ""}
        self.print(f"New Joystick connected: {added_joystick.name}({added_joystick.id})")
    ...
    def sendRawStream(self, stream, robot, *args, **kwargs):
        sample = self.convertTwiprSample(stream)
        self.sendStream(sample)
